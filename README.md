...
一个本地优先、可纯静态部署的 Three.js 粉丝向网页。首页把指定的星野爱 3D 模型作为全屏实时舞台，前景以七幕分页叙事覆盖；翻页时人物镜头、舞台色调和氛围同步平滑切换。独立的 `gallery.html` 保留模型分层、七机位和自由浏览能力。
...
`main.js` 只管理叙事状态：章节、导航、应援、设置、声音和无障碍公告；`stage.js` 只管理 Three.js：模型、镜头、灯光色彩、粒子、扫光和渲染循环。章节表为每幕定义名字、主题和机位，`goTo()` 同时更新 HTML 可见章节与 `Stage.shot()`，因此文字和镜头不会成为两套互不相干的轮播。
